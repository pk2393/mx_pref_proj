...
generated quantities {
  ...
  // randomly pick re_beta by prefecture from past values
  matrix[3, Ntime_fut*Npref] re_beta_pref_fut;
  for (n in 1:Npref) {
    re_beta_pref_fut[1, (n-1)*Ntime_fut + 1] = re_beta_pref[1, (n-1)*Ntime+20];
    re_beta_pref_fut[2, (n-1)*Ntime_fut + 1] = re_beta_pref[2, (n-1)*Ntime+20];
    re_beta_pref_fut[3, (n-1)*Ntime_fut + 1] = re_beta_pref[3, (n-1)*Ntime+20];
    
    re_beta_pref_fut[1, (n-1)*Ntime_fut + 2] = re_beta_pref[1, (n-1)*Ntime+20];
    re_beta_pref_fut[2, (n-1)*Ntime_fut + 2] = re_beta_pref[2, (n-1)*Ntime+20];
    re_beta_pref_fut[3, (n-1)*Ntime_fut + 2] = re_beta_pref[3, (n-1)*Ntime+20];
    
    re_beta_pref_fut[1, (n-1)*Ntime_fut + 3] = re_beta_pref[1, (n-1)*Ntime+20];
    re_beta_pref_fut[2, (n-1)*Ntime_fut + 3] = re_beta_pref[2, (n-1)*Ntime+20];
    re_beta_pref_fut[3, (n-1)*Ntime_fut + 3] = re_beta_pref[3, (n-1)*Ntime+20];
 }

  // extend the drift of 2018->2019 up to 2022
  vector[3] beta_mean_drift;
  for(p in 1:3){
    beta_mean_drift[p] = beta_mean[p, Ntime] - beta_mean[p, Ntime-1];
  }
